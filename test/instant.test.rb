...
test "instant class helpers and formatting" do
	...
	from_ruby_time = Literal::Instant.from_ruby_time(ruby_time)

	assert_equal ruby_time.to_i, from_ruby_time.unix_timestamp_in_seconds
	assert Literal::Instant === Literal::Instant.now
	assert_equal "Literal::Instant(1736798400.1234567)", from_ruby_time.inspect
end

test "instant parse arithmetic and zone conversion" do
	instant = Literal::Instant.parse("2025-01-13T20:00:00.123456789Z")
	duration = Literal::Duration.new(seconds: 1, subseconds: Rational(1, 10))
	...
	assert_equal 1_736_798_401, (instant + Literal::Duration.new(seconds: 1)).unix_timestamp_in_seconds
	assert_equal 1_736_798_399, (instant - Literal::Duration.new(seconds: 1)).unix_timestamp_in_seconds
	...
	assert instant.equals(Literal::Instant.parse("2025-01-13T20:00:00.123456789Z"))
	assert_equal(-1, Literal::Instant.compare(instant - Literal::Duration.new(seconds: 1), instant))
	assert_equal 1, instant.since(instant - Literal::Duration.new(seconds: 1)).seconds
	assert_equal 1, instant.until(instant + Literal::Duration.new(seconds: 1)).seconds
