...
test "local time to local date time helpers" do
	...
	assert_equal Literal::Duration.new(subseconds: Rational(1, 1)), time.since("09:15:29.123456789")
	assert_equal Literal::Duration.new(subseconds: Rational(1, 1)), time.until(date_time)
	...
	assert_equal Rational(60, 1), Literal::LocalTime.new(hour: 10).since(Literal::LocalTime.new(hour: 9, minute: 59)).subseconds
